...
class SchoolsApi:
    ...
    def get_school(
        ...
        include_ranges: Annotated[Optional[StrictBool], Field(description="true = return test-score percent metrics as the state reported them: a number when exact, otherwise an object with a status ('range', 'suppressed', 'legacyImputed' or 'derived') and, when available, value / low / high; rows the state reported only as ranges or suppressed values are included. false (default) = the record is identical to v2.4 (exact values only). See https://developer.schooldigger.com/data-quality (optional)")] = None,
        ...
        :type id: str
        :param include_ranges: true = return test-score percent metrics as the state reported them: a number when exact, otherwise an object with a status ('range', 'suppressed', 'legacyImputed' or 'derived') and, when available, value / low / high; rows the state reported only as ranges or suppressed values are included. false (default) = the record is identical to v2.4 (exact values only). See https://developer.schooldigger.com/data-quality (optional)
        :type include_ranges: bool
    ...
    def _get_school_serialize(
        ...
        if include_ranges is not None:
            ...
            _query_params.append(('includeRanges', include_ranges))
